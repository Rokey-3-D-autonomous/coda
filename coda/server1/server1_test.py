...
class Server1:
    ...
    def __init__(self):
        ...
        self.detect_node = DetectionNode(
            self.patrol_node.cancel
        )  # detection 했을 떄, inference 잠시 중단
    ...
    def patrol(self):
        # with self.lock:
        if self.server_state in [
            self.ServerState.IDLE,
            self.ServerState.DISPATCHING,
            self.ServerState.RECOVERY,
        ]:
            ...
            self.server_logger.error(
                f"[{self.patrol.__name__}] : {self.server_state} is wrong!!"
            )
            raise RuntimeError()
    ...
    def dispatch(self):
        ...
        if self.server_state in [self.ServerState.PATROLING, self.ServerState.RECOVERY]:
            ...
            self.server_logger.error(
                f"[{self.dispatch.__name__}] : {self.server_state} is wrong!!"
            )
            raise RuntimeError()
